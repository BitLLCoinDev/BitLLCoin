...
#if defined(NDEBUG)
# error "BitllCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6d67dfc30ff91fe71dc154f2bab1cca9aef364a7cfa8228695ce33337ab60a22");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BitllCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BitllCoin Signed Message:\n";
...
{
    // BitllCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BitllCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BitllCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1513768522, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "New decentralized cryptocurrency BitllCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d4fe05d123110cacf97dbe3033f2cb5d6b159aa6589a8ded320847d41400cbc97a32c37c11e915bd958114f11853c4844efa93b2147dbe4e3ecfc64e460dcc7f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1513768522;
        ...
        assert(block.hashMerkleRoot == uint256("0x7325ba89147fc850cff02295da0b2c7783aae84e43f8255d90952c870012e07b"));
...
unsigned char pchMessageStart[4] = { 0x7e, 0x16, 0x86, 0xee }; // BitllCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitllCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitllCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitllCoinMiner(CWallet *pwallet)
{
    printf("BitllCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitllcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BitllCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitllCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitllCoinMiner, pwallet));
}
